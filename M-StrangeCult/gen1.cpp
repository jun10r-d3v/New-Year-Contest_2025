...
void setting(int n,int lvlmx){
    ...
    // int key = rnd.next(2,5);
    int key = 3;
    int tmp = (limit[lvlmx+1]/key);
    // cout << "can range : " << tmp << "\n";
    for(int i=0;i<n;i++){
        if(people[i] == lvlmx){
            // cout << i+1 << " " << from[i+1] <<" ";
...
int main(int argc, char *argv[]){
    ...
    int n = rnd.next(5, 10);       // กำหนดจำนวนโหนด (สุ่มจาก 3 ถึง 10)
